...
internal class Program
{
    ...
    private static async Task<int> RunBrute(BruteOptions opts)
    {
        ...
        return await Run(opts.Keystore, opts.Parallelism, PasswordGenerator.BruteForce(charset, opts.Min, opts.Max), opts.GPU);
    }

    private static async Task<int> RunFile(FileOptions opts)
    {
        Console.WriteLine($"Словарь: {opts.Path}");
        return await Run(opts.Keystore, opts.Parallelism, File.ReadLines(opts.Path), opts.GPU);
    }
    ...
    private static string BuildCharset(BruteOptions opts)
    {
        ...
        if (opts.Chars != null)
        {
            sb.Append(opts.Chars);
        }

        return new string([.. sb.ToString().Distinct()]);
    }

    private static async Task<int> Run(string keystorePath, int parallelism, IEnumerable<string> passwords, bool useGpu)
    {
        ...
        var producer = Task.Run(() =>
        {
            try
            {
                ...
                {
                    byte[] privateKey;
                    try { privateKey = EthereumKeystore.Decrypt(keystore, item, useGpu); }
